...
class UserVerify extends React.Component<UserVerifyProps> {
  state = {
    ...
    // 审核中人数
    pengdingNum: this.props.data.filter(user => user.verifyState === 0).length,
    rejectNum: this.props.data.filter(user => user.verifyState === 1).length,
  };
  ...
  renderFooter = () => {
    const { total, pengdingNum, rejectNum } = this.state;
